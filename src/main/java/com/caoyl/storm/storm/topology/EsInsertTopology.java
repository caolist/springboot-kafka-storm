...
public class EsInsertTopology {
    ...
    public void runStorm(String[] args) {
        ...
        boltConf.put("es.input.json", app.getEsInputJson());
        ...
        conf.setDebug(true);
        conf.put("es.nodes", app.getEsNodes());
        conf.put("es.port", app.getEsPort());
        conf.put("es.index.auto.create", app.getEsIndexAutoCreate());
        conf.put("es.storm.bolt.write.ack", app.getEsStormBoltWriteAck());
        conf.put("es.storm.bolt.flush.entries.size", app.getEsStormBoltFlushEntriesSize());
